"""
From https://github.com/nickypro/welford-torch/blob/main/welford_torch/welford_torch.py
...
class Welford:
    ...
    def add_all(self, elements, backup_flg=False):
        ...
        if self.__shape is None:
            self.__device = (
                elements.device if (self.__device is None) else self.__device
            )
    ...
    def __getvars(self, ddof):
        ...
        if self.__count <= min_count:
            return torch.full(self.__shape, torch.nan, dtype=self.__dtype).to(
                self.__device
            )
        else:
